...
parser.add_argument('--student_checkpoint', default=r'D:\NLP\models_torch\uncased_L-6_H-768_A-12')
parser.add_argument('--train_data_path',
                    default=r'D:\PycharmProjects\style_change_detection2023\datum\style_change_detection_rearrange\train1.text')
parser.add_argument('--valid_data_path', default=None)
parser.add_argument('--test_data_path',
                    default=r'D:\PycharmProjects\style_change_detection2023\datum\style_change_detection_rearrange\validation1.text')
...
parser.add_argument('--teacher_batch_size', default=8, type=int)
...
def model_train(epoch_num, model, optimizer, args, loader_train, loader_valid=None):
    ...
    for step, batch in tqdm(enumerate(loader_train), total=len(loader_train)):
        ...
        logits = model(input_ids=input_ids,
                       ...
                       token_type_ids=token_type_ids)  # get model predictions for the current batch
...
def distill(epoch_num, teacher_model, student_model, student_optimizer, args, loader_train, loader_valid=None):
    ...
    for step, (input_ids, attention_mask, token_type_ids,labels) in tqdm(enumerate(loader_train), total=len(loader_train)):
        ...
        labels = labels.to(device)
        with torch.no_grad():
            teacher_logits = teacher_model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)
        ...
        if (step + 1) % args.output_per_steps == 0 and not step == 0 or (step + 1) == len(loader_train):
            if loader_valid is None:
                ...
                for i, (input_ids, attention_mask, token_type_ids, labels) in enumerate(loader_valid):
                    ...
                    labels = labels.to(device)
                    with torch.no_grad():
                        logits = student_model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)
                        val_loss = ce_loss_criterion(logits, labels)
                        total_val_loss += val_loss.item()
                    out = F.softmax(logits, dim=1)
                    out = out.argmax(dim=1)
...
def main():
    ...
    optimizer = AdamW(teacher.parameters(), lr=args.teacher_learning_rate)
    print(teacher)

    student = StudentModel(student_model, args).to(device)
    student_optimizer = AdamW(student.parameters(), lr=args.learning_rate)
    print(student)
