...
from transformers import AdamW, AutoTokenizer, BloomForSequenceClassification, BloomModel
...
# 给parser实例添加属性
parser.add_argument('--teacher_checkpoint', default=r'/T51/models/bloomz-560m')
parser.add_argument('--student_checkpoint', default=r'/T51/models/bloomz-560m')
parser.add_argument('--train_data_path', default=r'/T51/datasets/MSRP/msr_paraphrase_train.txt')
parser.add_argument('--valid_data_path', default=None)
parser.add_argument('--test_data_path', default=r'/T51/datasets/MSRP/msr_paraphrase_test.txt')
...
parser.add_argument('--learning_rate', default=6e-7)
parser.add_argument('--weight_decay', default=1e-4)
parser.add_argument('--output_per_steps', default=50)
parser.add_argument('--max_length', default=128, type=int)
parser.add_argument('--dropout_rate', default=0.1)
parser.add_argument('--batch_size', default=30)
parser.add_argument('--train_epochs', default=50)
parser.add_argument('--distill_epochs', default=10)
parser.add_argument('--teacher_weight_dir', default=r"/T51/hmj/style_change_detection2023/weights_t1")
parser.add_argument('--student_weight_dir', default=r"/T51/hmj/style_change_detection2023/weights_s1")
...
def model_train(epoch_num, model, optimizer, args, loader_train, loader_valid=None):
    ...
    for step, batch in tqdm(enumerate(loader_train), total=len(loader_train)):
        ...
        logits = model(input_ids=input_ids, attention_mask=attention_mask,
                       labels=labels).logits  # get model predictions for the current batch
...
def distill(epoch_num, teacher_model, student_model, args, loader_train, loader_valid=None):
    teacher_model.eval()
    student_model.train()
    student_optimizer = AdamW(student_model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)
...
def main():
    ...
    teacher = BloomForSequenceClassification.from_pretrained(args.teacher_checkpoint, num_labels=2).to(device)
    student = BloomForSequenceClassification.from_pretrained(args.student_checkpoint, num_labels=2).to(device)

    optimizer = AdamW(teacher.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)
    print(teacher)
    print(student)
