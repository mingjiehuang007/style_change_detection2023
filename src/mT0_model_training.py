...
from transformers import AdamW, Adafactor, AutoTokenizer, MT5EncoderModel, MT5Tokenizer
...
parser.add_argument('--student_checkpoint', default=r'/T51/models/mt0-large')
parser.add_argument('--train_data_path',
                    default=r'/T51/hmj/style_change_detection2023/datum/style_change_detection_rearrange/train2.text')
parser.add_argument('--valid_data_path', default=None)
parser.add_argument('--test_data_path',
                    default=r'/T51/hmj/style_change_detection2023/datum/style_change_detection_rearrange/validation2.text')
parser.add_argument('--train_valid_rate', default=0.8, type=float,
                    help='length of new train set / length of total train set')
parser.add_argument('--freeze_bert', default=False, type=bool)
# teacher hyperparameters
parser.add_argument('--teacher_learning_rate', default=1e-6, type=float)
parser.add_argument('--teacher_dropout_rate', default=0.1, type=float)
parser.add_argument('--teacher_batch_size', default=16, type=int)
parser.add_argument('--train_epochs', default=20, type=int)
# student hyperparameters
parser.add_argument('--learning_rate', default=3e-4, type=float)
parser.add_argument('--dropout_rate', default=0.1, type=float)
parser.add_argument('--batch_size', default=16, type=int)
parser.add_argument('--distill_epochs', default=20, type=int)
# shared hyperparameters
parser.add_argument('--max_length', default=256, type=int)
parser.add_argument('--output_per_steps', default=500, type=int)
parser.add_argument('--weight_decay', default=1e-4)
parser.add_argument('--teacher_weight_dir', default=r"/T51/hmj/style_change_detection2023/weights_t2")
parser.add_argument('--student_weight_dir', default=r"/T51/hmj/style_change_detection2023/weights_s2")
parser.add_argument('--temperature', default=4, type=int)
parser.add_argument('--alpha', default=0.7, type=float, help='ground truth weights')
# 传递属性给args实例
...
class TeacherModel(nn.Module):
    def __init__(self, model, args):
        ...
        self.model = model
        self.dropout = nn.Dropout(args.teacher_dropout_rate)
        self.relu = nn.ReLU()
...
def main():
    ...
    train_data = load_data(args.train_data_path)
    random_(train_data)
    valid_data = load_data(args.test_data_path)
    test_data = load_data(args.test_data_path)
    ...
    optimizer = Adafactor(teacher.parameters(), scale_parameter=False, relative_step=False, warmup_init=False,
                          lr=args.teacher_learning_rate, weight_decay=args.weight_decay)
    teacher.load_state_dict(torch.load('/T51/hmj/style_change_detection2023/teacher_weights/best_teacher_weights.pth'))
    ...
    loader_train = dataset_loader(MyDataset(train_data), args.teacher_batch_size)
    loader_valid = dataset_loader(MyDataset(valid_data), args.teacher_batch_size)
    loader_test = dataset_loader(MyDataset(test_data), args.teacher_batch_size)
    ...
    # distillation
    loader_train = dataset_loader(MyDataset(train_data), args.batch_size)
    loader_valid = dataset_loader(MyDataset(valid_data), args.batch_size)
    loader_test = dataset_loader(MyDataset(test_data), args.batch_size)
    avg_loss_min, accuracy_max = float('inf'), 0
